...
logger.info(f"   -> Chave encontrada? {'SIM' if settings.SUPABASE_SECRET_KEY else 'NAO'}")
if settings.SUPABASE_SECRET_KEY:
    logger.info(f"   -> Inicio da chave: {settings.SUPABASE_SECRET_KEY[:20]}...")
    logger.info(f"   -> Tamanho da chave: {len(settings.SUPABASE_SECRET_KEY)} caracteres")
    logger.info(f"   -> Tipo de chave: {'service_role' if 'service_role' in settings.SUPABASE_SECRET_KEY else 'anon'}")
